...
export function spawn(signal, fn) {
    ...
    let promise = new Promise((resolve, reject) => {
        ...
        fork(signal => fn(signal, {
            ...
            .join()
            .then(value => {
            spawnAbortController.abort(new AbortError('spawn() function finished', false));
            result = { value };
        }, error => {
            spawnAbortController.abort(isAbortError(error)
                ? error
        ...
        function fork(forkFn) {
            ...
            taskPromise
                ...
                spawnAbortController.abort(new AbortError('A function forked by spawn() threw', false));
            })
                .finally(() => {
                tasks.delete(task);
                if (tasks.size === 0) {
