...
export class MergeWithExisting {
    ...
    static hfresh(current, update) {
        return { ...current, ...update };
    }
    static dynamic(current, update) {
        if (!current) {
            return update;
        }
        current;
        ...
        const merged = { ...current, ...rest };
        if (hnsw) {
            merged.hnsw = MergeWithExisting.hnsw(current.hnsw, hnsw);
        }
        if (flat) {
            merged.flat = MergeWithExisting.flat(current.flat, flat);
        }
        return merged;
    }
