...
class ConnectionGRPC extends gql_js_1.default {
    ...
    search = (collection, consistencyLevel, tenant, abortSignal) => {
        if (this.authEnabled) {
            return this.login().then((token) => this.grpc.search(collection, consistencyLevel, tenant, `Bearer ${token}`, abortSignal));
        }
        return new Promise((resolve) => resolve(this.grpc.search(collection, consistencyLevel, tenant, undefined, abortSignal)));
    };
    tenants = (collection) => {
        if (this.authEnabled) {
...
const grpcClient = (config) => {
    ...
    return {
        ...
        search: (collection, consistencyLevel, tenant, bearerToken, abortSignal) => searcher_js_1.default.use(client, collection, getMetadataWithEmbeddingServiceAuth(config, bearerToken), config.timeout?.query || 30, consistencyLevel, tenant, abortSignal),
